...
The Teensy build is the **primary DAQ controller**: I2C sensor acquisition from the ADS1115 path, optional **SD logging**, and **LoRa** transmission of sampled telemetry.
...
3. **`sensor_dispatcher_init()`** — initializes **low** and **high** pressure ADS1115 helpers (`lib/SensorDispatcher`).  
4. **`daq_init()`** — DAQ sequence/tick state (`lib/DAQLoop`).  
5. **`SDWrite::init(&sd_buffer, "data.bin")`** — optional; failure prints a warning and continues.  
6. **`LoraModule`** on **`Serial5`** (see `loramodule.h`: Teensy uses fixed `HardwareSerial&` = `Serial5`). `begin()` + `configure()` with [`lora_config.h`](../../lib/LoRaModule/lora_config.h).  
...
| Library | Purpose |
|---------|---------|
| [`DAQLoop`](../../lib/DAQLoop) | Builds `SampleFrame` from configured sensors |
...
- **I2C addresses:** [`hwconfig.h`](../../lib/DataTypes/hwconfig.h).  
