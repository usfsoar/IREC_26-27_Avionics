...
private:
    ...
    // sensor_count(1)+timestamp(4)+seq(4)+valid(1)+status(1)+raw_adc[SENSOR_COUNT]*2
    static constexpr size_t MAX_SERIALIZED_HEADER = 1 + 4 + 4 + 1 + 1 + (SENSOR_COUNT * sizeof(uint16_t));
};
